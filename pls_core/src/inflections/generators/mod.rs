...
pub fn create_html_body(
    ...
    match pm.inflection_class {
        ...
        InflectionClass::Conjugation => {
            conjugation::create_html_body(&pm.pattern, &pm.stem, transliterate, &q)
        }
        InflectionClass::Declension => {
            declension::create_html_body(&pm.pattern, &pm.stem, transliterate, &q)
        }
        InflectionClass::DeclensionPron1st => {
            declension_pron_x::create_html_body("1st", &pm.pattern, &pm.stem, transliterate, &q)
        }
        InflectionClass::DeclensionPron2nd => {
            declension_pron_x::create_html_body("2nd", &pm.pattern, &pm.stem, transliterate, &q)
        }
        InflectionClass::DeclensionPronDual => {
            declension_pron_dual::create_html_body(&pm.pattern, &pm.stem, transliterate, &q)
        }
    }
}
